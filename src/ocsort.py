...
class KalmanBoxTracker(object):
    ...
    def __init__(self, point, delta_t=3, orig=True,constant=False):
        """
        ...
        #for wildtrack, using unconstant velocity model
        if not constant:
            dt = 1.0
            a_var = 1.0
            self.kf.Q = np.array([
        ...
        #orinal constant velocity model
            self.kf.Q[-1, -1] *= 0.01
            self.kf.Q[2:, 2:] *= 0.01
...
class OCSort(object):
    ...
    def update(self, output_results):
        ...
        if self.use_byte and len(dets_second) > 0 and unmatched_trks.shape[0] > 0:
            ...
            if dist_left.min() < self.e_dist_threshold:
                ...
                for m in matched_indices:
                    ...
                    if dist_left[m[0], m[1]] > self.sec_thresh:
                        continue
        ...
        if unmatched_dets.shape[0] > 0 and unmatched_trks.shape[0] > 0:
            ...
            dist_left = np.array(dist_left)
            # if dist_left.min() < self.e_dist_threshold:
            if dist_left.min() < self.sec_thresh:
                """
                    NOTE: by using a lower threshold, e.g., self.iou_threshold - 0.1, you may
                    get a higher performance especially on MOT17/MOT20 datasets. But we keep it
                    uniform here for simplicity
